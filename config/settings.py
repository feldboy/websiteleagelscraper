...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        ..., env="DATABASE_URL", description="PostgreSQL database connection URL"
    )

    # LLM Configuration
    openai_api_key: Optional[str] = Field(
        None, env="OPENAI_API_KEY", description="OpenAI API key for content generation"
    )
    ...
    openrouter_api_key: Optional[str] = Field(
        ...
        description="OpenRouter API key for content generation",
    )
    ...
    temperature: float = Field(
        0.7, env="TEMPERATURE", description="Temperature setting for LLM responses"
    )
    ...
    max_retries: int = Field(
        3, env="MAX_RETRIES", description="Maximum retry attempts for failed requests"
    )
    request_timeout: int = Field(
        30, env="REQUEST_TIMEOUT", description="HTTP request timeout in seconds"
    )
    ...
    metrics_enabled: bool = Field(
        True, env="METRICS_ENABLED", description="Enable performance metrics collection"
    )
    ...
    scraping_interval_hours: int = Field(
        4, env="SCRAPING_INTERVAL_HOURS", description="Hours between scraping cycles"
    )
...
if 'pytest' not in sys.modules and 'tests' not in sys.argv[0]:
    ...
    # For tests, we'll create a mock settings instance
    settings = None
