        ...
        **kwargs
        ...
        try:
            ...
            if response.success:
                ...
                self.usage_tracker.add_usage(response.tokens_used, response.cost_estimate)
                logger.info(
                ...
                if retry_count < self.max_retries and self._should_retry(response.error):
                    wait_time = 2 ** retry_count  # Exponential backoff
                    logger.info(f"Retrying LLM call in {wait_time}s (attempt {retry_count + 1})")
