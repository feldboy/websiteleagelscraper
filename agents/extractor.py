...
from models.extracted_data import (
    ExtractedData, LegalEntity, EntityType, MonetaryAmount, 
    LegalDate, CaseReference, LegalTopic, Jurisdiction, CourtLevel
)
...
class DateExtractor:
    ...
    def __init__(self):
        ...
        self.date_types = [
            'filing date', 'hearing date', 'trial date', 'deadline',
            'settlement date', 'ruling date', 'appeal date', 'discovery deadline'
        ]
        ...
        for pattern in self.date_patterns:
            ...
            for match in matches:
                ...
                if parsed_date:
                    ...
                    date_type = self._determine_date_type(text, match.start(), match.end())
...
class CaseExtractor:
                ...
                if ' v. ' in case_text or ' v ' in case_text:
                    ...
                    if len(parties) >= 2:
                        cases.append(CaseReference(
                            case_name=case_text,
                            parties=parties,
                            year=self._extract_year_from_context(text, match.start(), match.end())
                        ))
                elif 'Case No.' in case_text or 'Docket No.' in case_text:
                    # This is a case number
                    case_number = re.sub(r'(?:Case|Docket)\s+No\.?\s*', '', case_text, flags=re.IGNORECASE)
                    cases.append(CaseReference(
    ...
    def _extract_year_from_context(self, text: str, start: int, end: int) -> Optional[int]:
...
class ExtractionAgent:
    ...
    async def extract_article_data(self, article_id: str, content: str, title: str = "") -> ExtractedData:
        """
    ...
    async def _extract_with_llm(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await llm_client.generate_json(prompt, max_tokens=1000, temperature=0.3)
    ...
    def _normalize_llm_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized['key_quotes'] = response.get('key_quotes', [])[:5]  # Limit to 5 quotes
        normalized['practice_areas'] = response.get('practice_areas', [])
        normalized['structured_data'] = response.get('structured_data', {})
        
        return normalized
    
    def _merge_entities(self, pattern_entities: List[LegalEntity], llm_entities: List[LegalEntity]) -> List[LegalEntity]:
    ...
    def _determine_jurisdiction(self, text: str, entities: List[LegalEntity]) -> Optional[Jurisdiction]:
        """Determine legal jurisdiction from content."""
        text_lower = text.lower()
        
        # Check for federal indicators
        federal_indicators = ['federal court', 'supreme court', 'u.s. court', 'federal judge']
        ...
        international_indicators = ['international court', 'treaty', 'international law']
        if any(indicator in text_lower for indicator in international_indicators):
            return Jurisdiction.INTERNATIONAL
        
        return None
    
    def _determine_court_level(self, text: str, entities: List[LegalEntity]) -> Optional[CourtLevel]:
        """Determine court level from content."""
        text_lower = text.lower()
        
        if 'supreme court' in text_lower:
            return CourtLevel.SUPREME
        elif any(term in text_lower for term in ['court of appeals', 'appellate court']):
            return CourtLevel.APPELLATE
        elif any(term in text_lower for term in ['district court', 'trial court', 'superior court']):
        ...
        try:
            ...
            for article in articles:
                try:
                    ...
                    extraction_id = await database_agent.store_extraction(extracted_data)
                    processed_ids.append(extraction_id)
                    
                    logger.info(f"Processed article {article['id']} -> extraction {extraction_id}")
