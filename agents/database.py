...
from sqlalchemy import Column, String, DateTime, Text, Integer, Float, Boolean, JSON, Index
...
class ArticleRecord(Base):
    ...
    content_hash = Column(String(64), nullable=False, unique=True, index=True)  # For duplicate detection
...
class DatabaseAgent:
    ...
    async def store_article(self, scraped_data: ScrapedData) -> Optional[str]:
        ...
        async with self.get_session() as session:
            try:
                ...
                if existing.scalar():
                    logger.info(f"Duplicate content detected for URL: {scraped_data.url}")
                    return None
                ...
                if "content_hash" in str(e):
                    logger.info(f"Duplicate content hash detected for URL: {scraped_data.url}")
                    return None
                else:
        ...
        async with self.get_session() as session:
            try:
                ...
                article_exists = await session.execute(
                    select(ArticleRecord).where(ArticleRecord.id == extracted_data.article_id)
                )
                ...
                extraction = ExtractionRecord(
                    ...
                    monetary_amounts=[amount.dict() for amount in extracted_data.monetary_amounts],
                    legal_topics=[topic.value for topic in extracted_data.legal_topics],
                    key_quotes=extracted_data.key_quotes,
                    jurisdiction=extracted_data.jurisdiction.value if extracted_data.jurisdiction else None,
                    court_level=extracted_data.court_level.value if extracted_data.court_level else None,
                ...
                logger.info(f"Stored extraction: {extraction.id} for article {extracted_data.article_id}")
        ...
        async with self.get_session() as session:
            try:
                ...
                extraction_record = await session.execute(
                    select(ExtractionRecord).where(ExtractionRecord.id == quality.extraction_id)
                )
                ...
                logger.info(f"Stored quality assessment: {quality_record.id} for extraction {quality.extraction_id}")
    ...
    async def get_extraction_by_article_id(self, article_id: str) -> Optional[Dict[str, Any]]:
        """Get extraction data for a specific article."""
        async with self.get_session() as session:
            try:
                result = await session.execute(
                    select(ExtractionRecord).where(ExtractionRecord.article_id == article_id)
                )
    ...
    async def get_database_stats(self) -> Dict[str, int]:
        ...
        async with self.get_session() as session:
            try:
                # Count articles
                total_articles = await session.execute(select(func.count(ArticleRecord.id)))
                processed_articles = await session.execute(
                    select(func.count(ArticleRecord.id)).where(ArticleRecord.processed == True)
                )
                
                # Count extractions
                total_extractions = await session.execute(select(func.count(ExtractionRecord.id)))
                
                # Count quality assessments
                total_quality = await session.execute(select(func.count(QualityRecord.id)))
                approved_quality = await session.execute(
                    select(func.count(QualityRecord.id)).where(QualityRecord.approved == True)
                )
