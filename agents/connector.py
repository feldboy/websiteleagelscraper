...
class ArticleConnection(BaseModel):
    ...
    strength: float = Field(..., ge=0.0, le=1.0, description="Strength of connection (0-1)")
    description: str = Field(..., description="Description of the connection")
    
    # Supporting evidence
    shared_entities: List[str] = Field(default_factory=list, description="Shared legal entities")
    shared_topics: List[str] = Field(default_factory=list, description="Shared legal topics")
    shared_cases: List[str] = Field(default_factory=list, description="Shared case references")
    
    discovered_at: datetime = Field(default_factory=datetime.now, description="When connection was discovered")


class TrendAnalysis(BaseModel):
    """Analysis of legal trends across multiple articles."""
    
    trend_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique trend identifier")
    ...
    trend_direction: str = Field(..., description="Direction: emerging, growing, declining, stable")
    temporal_pattern: str = Field(..., description="Temporal pattern: recent, ongoing, cyclical")
    
    # Supporting data
    supporting_articles: List[str] = Field(..., description="Article IDs supporting this trend")
    key_indicators: List[str] = Field(..., description="Key indicators of the trend")
    affected_areas: List[str] = Field(..., description="Legal areas affected by the trend")
    
    # Impact assessment
    impact_level: str = Field(..., description="Impact level: low, medium, high")
    geographic_scope: str = Field(..., description="Geographic scope: local, state, national, international")
    
    # Timeline
    earliest_mention: Optional[datetime] = Field(None, description="Earliest mention in articles")
    latest_mention: Optional[datetime] = Field(None, description="Latest mention in articles")
...
class ConnectionAnalysis(BaseModel):
    ...
    # Connections
    direct_connections: List[ArticleConnection] = Field(default_factory=list, description="Direct article connections")
    thematic_clusters: List[Dict[str, Any]] = Field(default_factory=list, description="Thematic clusters of articles")
    
    # Trends
    identified_trends: List[TrendAnalysis] = Field(default_factory=list, description="Identified legal trends")
    ...
    analyzed_at: datetime = Field(default_factory=datetime.now, description="When analysis was performed")
    analysis_quality: float = Field(0.0, ge=0.0, le=1.0, description="Quality score of the analysis")
...
class SimilarityAnalyzer:
    ...
    def calculate_content_similarity(self, articles: List[Dict[str, str]]) -> np.ndarray:
    ...
    def find_entity_overlaps(self, article_extractions: List[Dict[str, Any]]) -> Dict[Tuple[int, int], Dict[str, List[str]]]:
        ...
        for i, extraction1 in enumerate(article_extractions):
            for j, extraction2 in enumerate(article_extractions[i+1:], i+1):
                ...
                entities1 = {entity['name'].lower() for entity in extraction1.get('entities', [])}
                entities2 = {entity['name'].lower() for entity in extraction2.get('entities', [])}
                ...
                # Find shared cases
                cases1 = {case['case_name'].lower() for case in extraction1.get('cases', [])}
                cases2 = {case['case_name'].lower() for case in extraction2.get('cases', [])}
    ...
    def calculate_temporal_proximity(self, articles: List[Dict[str, Any]]) -> Dict[Tuple[int, int], float]:
...
class TrendDetector:
    ...
    def detect_trends(self, articles: List[Dict[str, Any]], extractions: List[Dict[str, Any]]) -> List[TrendAnalysis]:
    ...
    def _analyze_topic_trends(self, articles: List[Dict[str, Any]], extractions: List[Dict[str, Any]]) -> List[TrendAnalysis]:
        ...
        for period, period_articles in time_periods.items():
            period_extractions = [extractions[i] for i, article in enumerate(articles) if article in period_articles]
        ...
        for topic, period_counts in topic_frequency.items():
            if len(period_counts) >= 2 and sum(period_counts.values()) >= self.min_articles_for_trend:
                # Calculate trend direction
                periods = sorted(period_counts.keys())
                early_count = sum(period_counts[p] for p in periods[:len(periods)//2])
                ...
                trend = TrendAnalysis(
                    ...
                    key_indicators=[f"Topic: {topic}", f"Article count: {sum(period_counts.values())}"],
    ...
    def _analyze_entity_trends(self, articles: List[Dict[str, Any]], extractions: List[Dict[str, Any]]) -> List[TrendAnalysis]:
        ...
        for entity_name, count in entity_counts.most_common(10):
            if count >= self.min_articles_for_trend:
                trend = TrendAnalysis(
                    ...
                    key_indicators=[f"Entity: {entity_name}", f"Mention count: {count}"],
    ...
    def _analyze_jurisdiction_trends(self, articles: List[Dict[str, Any]], extractions: List[Dict[str, Any]]) -> List[TrendAnalysis]:
        ...
        for jurisdiction, count in jurisdiction_counts.items():
            if count >= self.min_articles_for_trend:
                ...
                trend = TrendAnalysis(
                    ...
                    temporal_pattern="recent",
                    supporting_articles=[articles[i]['id'] for i, e in enumerate(extractions) if e.get('jurisdiction') == jurisdiction],
                    key_indicators=[f"Jurisdiction: {jurisdiction}", f"Activity: {percentage:.1f}%"],
    ...
    def _group_by_time_periods(self, articles: List[Dict[str, Any]], days_per_period: int = 7) -> Dict[str, List[Dict[str, Any]]]:
        """Group articles by time periods."""
...
class ConnectionAgent:
    ...
    async def analyze_article_connections(self, article_ids: List[str], time_window_days: int = 30) -> ConnectionAnalysis:
        """
        ...
        try:
            ...
            articles, extractions = await self._prepare_analysis_data(article_ids, time_window_days)
            
            if len(articles) < 2:
                logger.warning(f"Insufficient articles for connection analysis: {len(articles)}")
                return ConnectionAnalysis(article_ids=article_ids)
            
            # Calculate similarity matrix
            similarity_matrix = self.similarity_analyzer.calculate_content_similarity(articles)
            
            # Find entity overlaps
            entity_overlaps = self.similarity_analyzer.find_entity_overlaps(extractions)
            
            # Calculate temporal proximity
            temporal_proximities = self.similarity_analyzer.calculate_temporal_proximity(articles)
            
            # Generate connections
            connections = await self._generate_connections(
                articles, extractions, similarity_matrix, entity_overlaps, temporal_proximities
            )
            
            # Create thematic clusters
            clusters = self._create_thematic_clusters(articles, extractions, similarity_matrix)
            ...
            quality_score = self._calculate_analysis_quality(connections, clusters, enhanced_trends)
    ...
    async def _prepare_analysis_data(
        ...
        for article in db_articles:
            if article['scraped_at'] >= cutoff_date:
                ...
                # Get extraction data
                extraction = await database_agent.get_extraction_by_article_id(article['id'])
                if extraction:
    ...
    async def _generate_connections(
        ...
        for i in range(len(articles)):
            for j in range(i + 1, len(articles)):
                ...
                content_similarity = similarity_matrix[i, j] if similarity_matrix.size > 0 else 0.0
    ...
    def _calculate_entity_overlap_score(self, entity_overlap: Dict[str, List[str]]) -> float:
    ...
    def _create_thematic_clusters(
        ...
        for i in range(len(articles)):
            ...
            if len(cluster_articles) >= 2:  # Only create cluster if it has multiple articles
                cluster = {
                    ...
                    'cluster_strength': np.mean([
                        similarity_matrix[cluster_articles[x], cluster_articles[y]]
                        for x in range(len(cluster_articles))
                        for y in range(x + 1, len(cluster_articles))
                    ]) if len(cluster_articles) > 1 else 0.0
                }
    ...
    async def _get_llm_trend_analysis(self, trend_name: str, summaries: str) -> Optional[str]:
        ...
        try:
            ...
            response = await llm_client.generate(prompt, max_tokens=500, temperature=0.7)
    ...
    def _limit_connections_per_article(self, connections: List[ArticleConnection]) -> List[ArticleConnection]:
        """Limit the number of connections per article to avoid overcrowding."""
        article_connection_count = defaultdict(int)
        filtered_connections = []
        
        for connection in connections:
            if (article_connection_count[connection.article_id_1] < self.max_connections_per_article and
                article_connection_count[connection.article_id_2] < self.max_connections_per_article):
