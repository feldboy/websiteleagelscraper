...
class ArticleSummary(BaseModel):
    ...
    summary: str = Field(..., min_length=100, max_length=200, description="Concise article summary")
    key_points: List[str] = Field(..., min_items=2, max_items=5, description="Key points from the article")
    legal_significance: str = Field(..., min_length=50, description="Legal significance and implications")
    
    # Classification
    urgency_level: str = Field("medium", description="Urgency level: low, medium, high")
    impact_scope: str = Field("industry", description="Impact scope: local, state, national, international, industry")
    
    # Quality metrics
    summary_quality: float = Field(0.0, ge=0.0, le=1.0, description="Quality score of the summary")
    relevance_score: float = Field(0.0, ge=0.0, le=1.0, description="Relevance to legal audience")
    
    # Metadata
    summarized_at: datetime = Field(default_factory=datetime.now, description="When summary was created")
...
class SummaryQualityAssessment(BaseModel):
    ...
    clarity_score: float = Field(..., ge=0.0, le=10.0, description="Clarity and readability")
    accuracy_score: float = Field(..., ge=0.0, le=10.0, description="Factual accuracy")
    completeness_score: float = Field(..., ge=0.0, le=10.0, description="Coverage of key information")
    legal_precision_score: float = Field(..., ge=0.0, le=10.0, description="Legal terminology precision")
    
    # Overall assessment
    overall_score: float = Field(..., ge=0.0, le=10.0, description="Overall quality score")
    meets_standards: bool = Field(False, description="Whether summary meets quality standards")
    
    # Issues and recommendations
    issues_identified: List[str] = Field(default_factory=list, description="Issues found in summary")
...
class SummaryProcessor:
    ...
    def validate_summary_structure(self, summary: str, key_points: List[str]) -> List[str]:
        ...
        if word_count < self.min_word_count:
            issues.append(f"Summary too short: {word_count} words (min: {self.min_word_count})")
        elif word_count > self.max_word_count:
            issues.append(f"Summary too long: {word_count} words (max: {self.max_word_count})")
        
        # Check key points
        if len(key_points) < self.min_key_points:
            issues.append(f"Too few key points: {len(key_points)} (min: {self.min_key_points})")
        elif len(key_points) > self.max_key_points:
            issues.append(f"Too many key points: {len(key_points)} (max: {self.max_key_points})")
        ...
        legal_terms = [
            'court', 'judge', 'ruling', 'lawsuit', 'legal', 'law', 'case',
            'attorney', 'plaintiff', 'defendant', 'jurisdiction', 'statute'
        ]
    ...
    def assess_summary_quality(self, summary: ArticleSummary, original_content: str) -> SummaryQualityAssessment:
        ...
        completeness_score = self._assess_completeness(summary.summary, summary.key_points, original_content)
        legal_precision_score = self._assess_legal_precision(summary.summary)
        
        # Calculate overall score
        overall_score = (clarity_score + accuracy_score + completeness_score + legal_precision_score) / 4
    ...
    def _assess_clarity(self, summary: str) -> float:
        ...
        passive_indicators = len(re.findall(r'\b(?:was|were|been|being)\s+\w+ed\b', summary, re.IGNORECASE))
    ...
    def _assess_accuracy(self, summary: str, original_content: str) -> float:
        ...
        legal_terms = [
            'court', 'judge', 'ruling', 'lawsuit', 'case', 'attorney',
            'plaintiff', 'defendant', 'verdict', 'settlement', 'appeal'
        ]
    ...
    def _assess_completeness(self, summary: str, key_points: List[str], original_content: str) -> float:
        ...
        important_concepts = [
            'what happened', 'legal issue', 'parties involved', 'outcome',
            'implications', 'precedent', 'ruling', 'decision'
        ]
        ...
        avg_key_point_length = sum(len(kp.split()) for kp in key_points) / max(len(key_points), 1)
    ...
    def _assess_legal_precision(self, summary: str) -> float:
        ...
        legal_terms = [
            'alleged', 'plaintiff', 'defendant', 'court', 'ruling', 'jurisdiction',
            'statute', 'regulation', 'precedent', 'appeal', 'motion', 'brief'
        ]
    ...
    def _generate_improvement_suggestions(
        ...
        if clarity_score < 7.0:
            suggestions.append("Simplify sentence structure and reduce complex punctuation")
        
        if accuracy_score < 7.0:
            suggestions.append("Ensure key legal terms and facts from original article are preserved")
        
        if completeness_score < 7.0:
            suggestions.append("Include more comprehensive coverage of the legal issue and its implications")
        
        if legal_precision_score < 7.0:
            suggestions.append("Use more precise legal terminology and avoid speculative language")
...
class SummarizerAgent:
    ...
    async def summarize_article(self, article_id: str, title: str, content: str) -> ArticleSummary:
        """
        ...
        try:
            ...
            summary.relevance_score = self._calculate_relevance_score(content, summary_text)
            
            # If quality is too low, attempt to improve
            if not quality_assessment.meets_standards:
                logger.warning(f"Summary quality below standards for article {article_id}, attempting improvement")
                improved_summary = await self._improve_summary(summary, quality_assessment, full_content)
                if improved_summary:
    ...
    async def _generate_summary_with_llm(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await llm_client.generate_json(structured_prompt, max_tokens=800, temperature=0.5)
            return response
            
        except Exception as e:
            logger.warning(f"Structured LLM summarization failed: {e}, falling back to text generation")
            
            # Fallback to text generation
            prompt = LegalPrompts.get_summarization_prompt(content)
            text_response = await llm_client.generate(prompt, max_tokens=400, temperature=0.5)
            
            if text_response.success:
                return {
                    'summary': text_response.content,
                    'key_points': ['Key information extracted', 'Legal implications noted'],
    ...
    async def _improve_summary(
        ...
        try:
            ...
            improved_response = await llm_client.generate_json(improvement_prompt, max_tokens=600, temperature=0.3)
            ...
            improved_summary = ArticleSummary(
                ...
                legal_significance=improved_response.get('legal_significance', summary.legal_significance),
                urgency_level=summary.urgency_level,
                impact_scope=summary.impact_scope
            )
            
            # Re-assess quality
            new_assessment = self.processor.assess_summary_quality(improved_summary, original_content)
            
            if new_assessment.overall_score > assessment.overall_score:
                logger.info(f"Summary improved: {assessment.overall_score:.1f} -> {new_assessment.overall_score:.1f}")
                improved_summary.summary_quality = new_assessment.overall_score / 10.0
                return improved_summary
            else:
                logger.warning("Summary improvement attempt did not yield better quality")
                return None
    ...
    def _determine_urgency(self, content: str, summary_data: Dict[str, Any]) -> str:
        ...
        high_urgency_terms = [
            'breaking', 'urgent', 'emergency', 'immediate', 'injunction',
            'restraining order', 'preliminary ruling', 'emergency motion'
        ]
        
        # Medium urgency indicators
        medium_urgency_terms = [
            'ruling', 'decision', 'verdict', 'settlement', 'appeal filed',
            'motion granted', 'court order'
        ]
        ...
        if high_count > 0 or any('urgent' in indicator.lower() for indicator in urgency_indicators):
            return 'high'
        elif medium_count > 1:
            return 'medium'
        else:
    ...
    def _determine_impact_scope(self, content: str, summary_data: Dict[str, Any]) -> str:
        ...
        if any(term in content_lower for term in ['international', 'treaty', 'foreign', 'global']):
            return 'international'
        
        # National scope indicators
        if any(term in content_lower for term in ['federal', 'supreme court', 'congress', 'national']):
            return 'national'
        
        # State scope indicators
        if any(term in content_lower for term in ['state', 'governor', 'legislature', 'state court']):
            return 'state'
        
        # Local scope indicators
        if any(term in content_lower for term in ['local', 'city', 'county', 'municipal']):
            return 'local'
    ...
    def _calculate_relevance_score(self, content: str, summary: str) -> float:
        ...
        legal_terms = [
            'court', 'judge', 'attorney', 'lawsuit', 'ruling', 'legal', 'law',
            'case', 'plaintiff', 'defendant', 'jurisdiction', 'statute', 'regulation'
        ]
        ...
        professional_topics = [
            'precedent', 'litigation', 'compliance', 'regulatory', 'contract',
            'intellectual property', 'corporate law', 'securities'
        ]
        
        professional_count = sum(1 for topic in professional_topics if topic in summary_lower)
        if professional_count > 0:
            score += 0.2
        
        return min(1.0, score)
    
    async def batch_summarize_articles(self, article_ids: List[str]) -> List[ArticleSummary]:
