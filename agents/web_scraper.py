...
class ContentExtractor:
    ...
    @staticmethod
    def extract_content(html: str, config: SourceConfig, url: str) -> Optional[ScrapedData]:
        """
        ...
        try:
            ...
            if not content_element:
                logger.warning(f"No content found with selector {config.content_selector} for {url}")
                return None
    ...
    def _passes_filters(content: str, title: str, config: SourceConfig) -> bool:
        ...
        if config.include_keywords:
            return any(keyword.lower() in full_text for keyword in config.include_keywords)
...
class WebScraperAgent:
    ...
    async def scrape_source(self, source_name: str, max_articles: Optional[int] = None) -> ScrapingBatch:
        """
    ...
    async def _discover_article_urls(
        ...
        async with WebScraper() as scraper:
            ...
            if not result.success:
                logger.error(f"Failed to scrape listing page {listing_url}: {result.error}")
                return []
    ...
    async def _scrape_article_batch(self, batch: ScrapingBatch, config: SourceConfig) -> None:
    ...
    async def _scrape_and_process_article(self, url: str, config: SourceConfig) -> ScrapingResult:
        ...
        async with WebScraper() as scraper:
            ...
            ).total_seconds() if scraping_result.scraped_at else 0.0
    ...
    async def scrape_all_sources(self, max_articles_per_source: Optional[int] = None) -> List[ScrapingBatch]:
        """Scrape all configured legal news sources."""
        sources = legal_sources.get_active_sources()
        ...
        for source_config in sources:
            try:
                max_articles = max_articles_per_source or source_config.max_articles_per_cycle
                batch = await self.scrape_source(source_config.name, max_articles)
