...
class ScheduledTask(BaseModel):
    ...
    task_type: str = Field(..., description="Type of task (workflow, maintenance, etc.)")
    ...
    # Execution tracking
    created_at: datetime = Field(default_factory=datetime.now, description="When task was created")
...
class WorkflowManager:
    ...
    async def execute_full_research_workflow(
        ...
        try:
            ...
            execution.articles_scraped = sum(batch.successful_urls for batch in scraping_batches)
            
            scraping_duration = (datetime.now() - scraping_start).total_seconds()
            supervisor_agent.update_performance_metrics("scraping", True, scraping_duration)
            ...
            supervisor_agent.update_performance_metrics("processing", True, extraction_duration)
            ...
            summaries = await summarizer_agent.batch_summarize_articles(processed_ids[:5])
            ...
            supervisor_agent.update_performance_metrics("generation", True, generation_duration)
            ...
            if validation_result.passed:
                ...
                try:
                    ...
                    if distribution_result.success:
                        ...
                        supervisor_agent.update_performance_metrics("distribution", False)
                ...
                logger.warning("Article failed quality validation, skipping distribution")
                execution.results['distribution'] = {
                    'distribution_success': False,
            ...
            logger.info(f"Full research workflow completed successfully: {execution_id}")
            logger.info(f"Results: {execution.articles_scraped} scraped, "
    ...
    async def execute_maintenance_workflow(self) -> WorkflowExecution:
        ...
        try:
            ...
            execution.results['log_cleanup'] = {
                'logs_cleaned': False  # Placeholder
            }
...
class SchedulerAgent:
    ...
    async def schedule_research_workflow(
        ...
        logger.info(f"Scheduled research workflow: every {interval_hours} hours, "
    ...
    async def schedule_maintenance_tasks(
        ...
        logger.info(f"Scheduled maintenance tasks: daily at {hour:02d}:{minute:02d} UTC")
    ...
    async def _execute_research_workflow(self, max_articles_per_source: int):
        ...
        try:
            ...
            if execution.success:
                logger.info(f"Scheduled research workflow completed successfully: {execution.execution_id}")
            else:
    ...
    async def _execute_health_check(self):
        ...
        try:
            ...
            if health_report.critical_issues:
                logger.error(f"Health check found critical issues: {health_report.critical_issues}")
    ...
    async def _execute_maintenance(self):
        ...
        try:
            ...
            if execution.success:
                logger.info(f"Scheduled maintenance completed successfully: {execution.execution_id}")
            else:
    ...
    async def execute_workflow_now(
        self,
        workflow_type: str = "full_research",
