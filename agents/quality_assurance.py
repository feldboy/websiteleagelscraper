...
class QualityStandards(BaseModel):
    ...
    min_legal_terminology_density: float = Field(0.02, description="Minimum legal terminology density")
    max_readability_complexity: float = Field(0.8, description="Maximum readability complexity")
    ...
    requires_legal_citations: bool = Field(False, description="Whether legal citations are required")
    requires_jurisdiction_mention: bool = Field(True, description="Whether jurisdiction must be mentioned")
    min_legal_entities: int = Field(2, description="Minimum number of legal entities mentioned")
...
class ValidationResult(BaseModel):
    ...
    word_count_compliant: bool = Field(..., description="Word count within acceptable range")
    originality_score: float = Field(..., ge=0.0, le=1.0, description="Originality score")
    legal_quality_score: float = Field(..., ge=0.0, le=10.0, description="Legal content quality")
    readability_score: float = Field(..., ge=0.0, le=10.0, description="Readability score")
    factual_accuracy_score: float = Field(..., ge=0.0, le=10.0, description="Factual accuracy score")
    
    # Content analysis
    plagiarism_detected: bool = Field(False, description="Whether plagiarism was detected")
    inappropriate_content: bool = Field(False, description="Whether inappropriate content found")
    legal_terminology_adequate: bool = Field(..., description="Adequate legal terminology usage")
    ...
    warnings: List[str] = Field(default_factory=list, description="Warnings and minor issues")
    ...
    def is_publishable(self) -> bool:
        ...
        return (
            self.passed and
            not self.plagiarism_detected and
            not self.inappropriate_content and
...
class PlagiarismDetector:
    ...
    async def check_plagiarism(self, content: str, title: str) -> Tuple[bool, float, List[str]]:
        """
        ...
        try:
            ...
            logger.info(f"Plagiarism check completed: plagiarized={is_plagiarized}, similarity={max_similarity:.2f}")
        ...
        try:
            ...
            for article in recent_articles:
                ...
                if similarity > self.similarity_threshold:
                    matching_sources.append(f"Similar to article: {article.get('title', 'Unknown')[:50]}...")
...
class ContentValidator:
    ...
    def _load_legal_terminology(self) -> List[str]:
        ...
        return [
            'court', 'judge', 'justice', 'ruling', 'decision', 'case', 'lawsuit',
            'litigation', 'attorney', 'lawyer', 'counsel', 'plaintiff', 'defendant',
            'jurisdiction', 'statute', 'regulation', 'law', 'legal', 'precedent',
            'appeal', 'motion', 'brief', 'verdict', 'settlement', 'injunction',
            'contract', 'agreement', 'liability', 'damages', 'evidence', 'testimony',
            'constitutional', 'federal', 'state', 'municipal', 'compliance',
            'violation', 'penalty', 'fine', 'sentence', 'conviction', 'acquittal'
        ]
    ...
    def validate_article(self, article: GeneratedArticle) -> ValidationResult:
        ...
        """
        validation_id = f"val_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{article.article_id[:8]}"
        ...
        content_appropriateness = self._validate_content_appropriateness(article.content)
        ...
        originality_score = article.originality_score if hasattr(article, 'originality_score') else 0.8
        ...
        passed = (
            ...
            originality_score >= self.standards.min_originality_score
        )
    ...
    def _validate_word_count(self, content: str) -> Dict[str, Any]:
        ...
        compliant = self.standards.min_word_count <= word_count <= self.standards.max_word_count
    ...
    def _validate_legal_quality(self, content: str) -> Dict[str, Any]:
        ...
        legal_term_count = sum(1 for term in self.legal_terminology if term in content_lower)
        ...
        pattern_count = sum(1 for pattern in legal_patterns if re.search(pattern, content_lower))
        ...
        jurisdiction_mentioned = any(
            term in content_lower for term in ['federal', 'state', 'local', 'court', 'jurisdiction']
        )
        ...
        adequate_terminology = legal_density >= self.standards.min_legal_terminology_density
        
        recommendations = []
        if legal_density < self.standards.min_legal_terminology_density:
            recommendations.append("Include more legal terminology and concepts")
        if pattern_count < 2:
            recommendations.append("Add more specific legal analysis and case discussion")
    ...
    def _validate_structure(self, content: str) -> Dict[str, Any]:
        ...
        has_intro = any(re.search(pattern, content, re.IGNORECASE) for pattern in intro_patterns)
...
class QualityAssuranceAgent:
    ...
    async def validate_article(self, article: GeneratedArticle) -> ValidationResult:
        ...
        """
        try:
            logger.info(f"Starting quality assurance validation for article {article.article_id}")
            
            # Basic content validation
            validation_result = self.content_validator.validate_article(article)
            
            # Plagiarism detection
            is_plagiarized, similarity_score, matching_sources = await self.plagiarism_detector.check_plagiarism(
            ...
            if is_plagiarized:
                ...
                validation_result.critical_issues.extend(matching_sources[:3])  # Limit to 3 sources
                validation_result.passed = False
            
            # LLM-enhanced quality assessment
            llm_assessment = await self._get_llm_quality_assessment(article)
            if llm_assessment:
                validation_result = self._merge_llm_assessment(validation_result, llm_assessment)
    ...
    async def _get_llm_quality_assessment(self, article: GeneratedArticle) -> Optional[Dict[str, Any]]:
        """Get quality assessment from LLM."""
        try:
            prompt = LegalPrompts.get_quality_prompt(f"Title: {article.title}\n\nContent: {article.content}")
            
            response = await llm_client.generate_json(prompt, max_tokens=600, temperature=0.3)
    ...
    def _merge_llm_assessment(self, validation_result: ValidationResult, llm_assessment: Dict[str, Any]) -> ValidationResult:
        """Merge LLM assessment with validation result."""
        try:
            # Update scores with LLM input
            llm_overall = llm_assessment.get('overall_score', validation_result.overall_score)
            validation_result.overall_score = (validation_result.overall_score + llm_overall) / 2
            
            # Add LLM-identified issues
            llm_issues = llm_assessment.get('issues_found', [])
            validation_result.warnings.extend(llm_issues[:3])  # Limit to 3 additional issues
            ...
            if llm_confidence:
                validation_result.confidence = (validation_result.confidence + llm_confidence) / 2
    ...
    async def batch_validate_articles(self, articles: List[GeneratedArticle]) -> List[ValidationResult]:
    ...
    def get_validation_statistics(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        plagiarism_detected = sum(1 for result in validation_results if result.plagiarism_detected)
        
        avg_overall_score = sum(result.overall_score for result in validation_results) / total_articles
        avg_originality = sum(result.originality_score for result in validation_results) / total_articles
        ...
        return {
            ...
            'plagiarism_rate': plagiarism_detected / total_articles if total_articles > 0 else 0,
