...
class TelegramMessage(BaseModel):
    ...
    # Delivery tracking
    created_at: datetime = Field(default_factory=datetime.now, description="When message was created")
    ...
    telegram_message_id: Optional[int] = Field(None, description="Telegram's message ID")
...
class TelegramFormatter:
    ...
    async def format_article_for_telegram(self, article: GeneratedArticle, validation: Optional[ValidationResult] = None) -> TelegramMessage:
        """
    ...
    def _create_telegram_headline(self, title: str) -> str:
        """Create an engaging headline with appropriate emojis."""
        # Add emoji based on content
        title_lower = title.lower()
        emoji = '🏛️'  # Default legal emoji
        
        for keyword, emoji_char in self.legal_emojis.items():
    ...
    async def _format_article_content(self, article: GeneratedArticle) -> str:
        """Format the main article content with Telegram markdown."""
        try:
            # Use LLM to format for Telegram
            formatted_content = await self._get_llm_telegram_format(article)
            
            if formatted_content:
                return formatted_content
            else:
                # Fallback to manual formatting
                return self._manual_format_content(article)
                
        except Exception as e:
            logger.warning(f"LLM formatting failed, using manual formatting: {e}")
            return self._manual_format_content(article)
    
    async def _get_llm_telegram_format(self, article: GeneratedArticle) -> Optional[str]:
        """Use LLM to format article for Telegram."""
        try:
            prompt = LegalPrompts.get_telegram_prompt(f"Title: {article.title}\n\nContent: {article.content}")
    ...
    def _manual_format_content(self, article: GeneratedArticle) -> str:
        ...
        if hasattr(article, 'quality_score') and article.quality_score > 0:
            quality_text = "✅ *High Quality*" if article.quality_score > 0.8 else "📊 *Verified Content*"
    ...
    def _add_quality_indicators(self, validation: ValidationResult) -> str:
        """Add quality indicators to the message."""
        ...
        if validation.overall_score >= 8.0:
            indicators.append("⭐ *Premium Quality*")
        elif validation.overall_score >= 7.0:
            indicators.append("✅ *Quality Verified*")
        
        if validation.originality_score >= 0.95:
            indicators.append("🆕 *Original Content*")
        
        if validation.legal_quality_score >= 8.0:
            indicators.append("⚖️ *Expert Analysis*")
    ...
    def _create_footer(self, article: GeneratedArticle) -> str:
        """Create message footer with metadata."""
        footer_parts = []
        
        # Word count
        footer_parts.append(f"📄 {article.word_count} words")
        
        # Timestamp
        timestamp = datetime.now().strftime("%B %d, %Y")
        footer_parts.append(f"📅 {timestamp}")
        
        # Author attribution
        footer_parts.append(f"🤖 {article.author}")
        
        footer = " • ".join(footer_parts)
        
        return f"\n\n─────────────────\n{footer}\n\n#LegalNews #LegalAnalysis #ProfessionalInsights"
    ...
    def _truncate_message(self, content: str) -> str:
        ...
        truncate_at = self.max_message_length - 100  # Leave room for truncation notice
        
        # Look for sentence ending before truncation point
        sentences_end = content.rfind('.', 0, truncate_at)
        if sentences_end > truncate_at * 0.8:  # If we found a good break point
            truncate_at = sentences_end + 1
        
        truncated = content[:truncate_at].strip()
...
class TelegramBotAgent:
    ...
    async def distribute_article(
        ...
        try:
            ...
            telegram_message = await self.formatter.format_article_for_telegram(article, validation)
            ...
            telegram_message.delivery_status = "sent" if delivery_result.success else "failed"
            ...
            if delivery_result.success:
                logger.info(f"Article distributed successfully: Telegram message ID {delivery_result.telegram_message_id}")
            else:
                logger.error(f"Article distribution failed: {delivery_result.error_message}")
        ...
        for attempt in range(self.max_retry_attempts):
            try:
                async with TelegramAPI() as api:
                    ...
                    retry_delay = result.retry_after or (self.retry_delay_base * (2 ** attempt))
                    logger.warning(
    ...
    def _should_retry(self, result: DeliveryResult, attempt: int) -> bool:
        ...
        if result.error_code and result.error_code.startswith("4") and result.error_code != "429":
            return False
        ...
        try:
            async with TelegramAPI() as api:
                ...
                if not test_results['bot_connection']:
                    test_results['error_messages'].append("Bot token is invalid or bot is not accessible")
                    return test_results
                ...
                if channel_info:
                    ...
                    test_results['error_messages'].append("Cannot access the specified channel/chat")
                ...
                if not test_result.success:
                    test_results['error_messages'].append(f"Cannot send messages: {test_result.error_message}")
