...
class ScrapedData(BaseModel):
    ...
    excerpt: Optional[str] = Field(
        None, max_length=500, description="Article excerpt/summary"
    )
    ...
    language: Optional[str] = Field(
        None, max_length=10, description="Detected language code"
    )
...
class ScrapingResult(BaseModel):
    ...
    attempt_number: int = Field(
        1, ge=1, le=10, description="Attempt number (for retries)"
    )
    ...
    robots_txt_allowed: Optional[bool] = Field(
        None, description="Whether robots.txt allowed scraping"
    )
    ...
    def is_retryable(self) -> bool:
        ...
        non_retryable_statuses = [ScrapingStatus.ROBOTS_BLOCKED]  # Permanent block
...
class ScrapingBatch(BaseModel):
    ...
    urls: list[HttpUrl] = Field(
        ..., description="URLs to scrape in this batch"
    )

    # Timing
    created_at: datetime = Field(
        default_factory=datetime.now, description="When batch was created"
    )
    started_at: Optional[datetime] = Field(
        None, description="When batch processing started"
    )
    completed_at: Optional[datetime] = Field(
        None, description="When batch processing completed"
    )

    # Progress tracking
    total_urls: int = Field(..., ge=0, description="Total number of URLs in batch")
    completed_urls: int = Field(0, ge=0, description="Number of URLs completed")
    successful_urls: int = Field(
        0, ge=0, description="Number of successfully scraped URLs"
    )
    failed_urls: int = Field(0, ge=0, description="Number of failed URLs")

    # Results
    results: list[ScrapingResult] = Field(
        default_factory=list, description="Results for each URL"
    )
