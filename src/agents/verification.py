...
class ContentValidator:
    ...
    def validate_scraped_data(self, data: ScrapedData) -> ValidationResult:
        ...
        if word_count < self.min_word_count:
            ...
            warnings.append(
                f"Content too short: {word_count} words (min: {self.min_word_count})"
            )
        ...
        suspicious_indicators = self._detect_suspicious_content(
        ...
        confidence_score = self._calculate_confidence_score(
            issues, quality_score, metadata_score
        )
    ...
    def _check_encoding_issues(self, content: str) -> List[str]:
        ...
        for indicator in encoding_indicators:
            if indicator in content:
                issues.append(
                    f"Potential encoding issue: found '{indicator}' character"
                )
    ...
    def _generate_recommendations(
        self, issues: List[ValidationIssue], metadata: Dict[str, Any]
    ) -> List[str]:
        ...
        if ValidationIssue.CONTENT_TOO_SHORT in issues:
            recommendations.append(
                "Consider scraping from a different URL or checking content selectors"
            )

        if ValidationIssue.LOW_QUALITY_CONTENT in issues:
            recommendations.append(
                "Review content extraction selectors to avoid navigation/footer content"
            )

        if ValidationIssue.ENCODING_ISSUES in issues:
            recommendations.append(
                "Check encoding settings and HTML parsing configuration"
            )

        if ValidationIssue.SUSPICIOUS_CONTENT in issues:
            recommendations.append(
                "Verify source URL and check for paywall or access restrictions"
            )

        if ValidationIssue.MISSING_METADATA in issues:
            recommendations.append(
                "Review metadata extraction selectors for author, date, etc."
            )
        ...
        if quality_score < 0.7:
            recommendations.append(
                "Consider improving content extraction to filter out non-article content"
            )

        return recommendations

    def _calculate_confidence_score(
        self, issues: List[ValidationIssue], quality_score: float, metadata_score: float
    ) -> float:
...
class VerificationAgent:
    ...
    async def verify_scraping_result(self, result: ScrapingResult) -> ValidationResult:
        ...
        if not result.success or not result.data:
            return ValidationResult(
                ...
                recommendations=[
                    "Check scraping configuration and source availability"
                ],
                metadata={},
            )
    ...
    async def should_retry_scraping(self, result: ScrapingResult) -> Tuple[bool, str]:
        ...
        if not validation.is_valid:
            ...
            has_critical_issues = any(
                issue in critical_issues for issue in validation.issues
            )
    ...
    async def batch_verify_articles(
