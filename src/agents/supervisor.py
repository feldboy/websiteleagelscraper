...
class AgentStatus(BaseModel):
    ...
    response_time: Optional[float] = Field(
        None, description="Last response time in seconds"
    )
...
class SystemHealthReport(BaseModel):
    ...
    system_metrics: Dict[str, Any] = Field(
        default_factory=dict, description="System-wide metrics"
    )
    database_metrics: Dict[str, Any] = Field(
        default_factory=dict, description="Database metrics"
    )
    ...
    def get_healthy_agent_count(self) -> int:
        ...
        return sum(
            1 for agent in self.agent_statuses if agent.status == HealthStatus.HEALTHY
        )

    def get_critical_agent_count(self) -> int:
        """Get number of agents in critical state."""
        return sum(
            1 for agent in self.agent_statuses if agent.status == HealthStatus.CRITICAL
        )
    ...
    async def check_database_agent(self) -> AgentStatus:
        ...
        try:
            ...
            db_stats = await asyncio.wait_for(
                database_agent.get_database_stats(), timeout=self.check_timeout
            )
            ...
            status.status_message = (
                f"Database health check timed out after {self.check_timeout}s"
            )
        ...
        try:
            ...
            if len(batch_statuses) == 0:
                ...
                if running_batches:
                    status.status = HealthStatus.HEALTHY
                    status.status_message = (
                        f"Web scraper active: {len(running_batches)} batches running"
                    )
                else:
                    status.status = HealthStatus.HEALTHY
                    status.status_message = (
                        f"Web scraper completed {len(batch_statuses)} recent batches"
                    )
                status.success_count = 1
    ...
    async def check_telegram_bot_agent(self) -> AgentStatus:
        ...
        try:
            ...
            delivery_stats = await asyncio.wait_for(
                telegram_bot_agent.get_delivery_statistics(), timeout=self.check_timeout
            )
            ...
            if delivery_stats:
                ...
                status.status = HealthStatus.WARNING
                status.status_message = (
                    "Telegram bot responded but no statistics available"
                )

        except asyncio.TimeoutError:
            status.status = HealthStatus.CRITICAL
            status.status_message = (
                f"Telegram bot check timed out after {self.check_timeout}s"
            )
            status.error_count = 1
            status.last_error = "Timeout"
        except Exception as e:
...
class AlertManager:
    ...
    def check_alerts(self, health_report: SystemHealthReport) -> List[Dict[str, Any]]:
        ...
        critical_agents = [
            agent
            for agent in health_report.agent_statuses
...
class SupervisorAgent:
    ...
    async def generate_health_report(self) -> SystemHealthReport:
        ...
        try:
            ...
            logger.info(
                f"Health report generated: {overall_status.value} status, "
    ...
    def _determine_overall_status(
        self, agent_statuses: List[AgentStatus], system_metrics: Dict[str, Any]
    ) -> HealthStatus:
        """Determine overall system health status."""
        # Check for critical agents
        critical_agents = [
            agent for agent in agent_statuses if agent.status == HealthStatus.CRITICAL
        ]
        if critical_agents:
        ...
        warning_agents = [
            agent for agent in agent_statuses if agent.status == HealthStatus.WARNING
        ]
    ...
    def _analyze_issues(
        ...
        if performance_metrics:
            ...
            if success_rate < 0.7:
                ...
                recommendations.append(
                    "Monitor error patterns and consider improvements"
                )
    ...
    def update_performance_metrics(
        self, operation_type: str, success: bool, duration: float = 0.0
    ):
            ...
            if cpu_percent > 0:
                ...
                if total_ops > 0:
                    self.performance_metrics.avg_cpu_usage = (
                        current_avg * (total_ops - 1) + cpu_percent
                    ) / total_ops
                else:
    ...
    def get_monitoring_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_health_check": (
                self.last_health_report.generated_at
