...
class LegalEntity(BaseModel):
    ...
    relevance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Relevance to the article (0-1)"
    )

    # Optional additional information
    title: Optional[str] = Field(
        None, max_length=100, description="Professional title or position"
    )
    organization: Optional[str] = Field(
        None, max_length=200, description="Associated organization"
    )
    location: Optional[str] = Field(
        None, max_length=100, description="Location/jurisdiction"
    )
...
class MonetaryAmount(BaseModel):
    ...
    currency: str = Field(
        "USD", max_length=3, description="Currency code (e.g., USD, EUR)"
    )
    formatted_string: str = Field(
        ..., description="Original formatted string from text"
    )
    context: Optional[str] = Field(
        None, description="Context where amount was mentioned"
    )
...
class LegalDate(BaseModel):
    ...
    date_type: str = Field(
        ..., description="Type of date (filing, hearing, deadline, etc.)"
    )
    context: Optional[str] = Field(None, description="Context where date was mentioned")
    confidence: float = Field(
        1.0, ge=0.0, le=1.0, description="Confidence in date extraction"
    )
...
class ExtractedData(BaseModel):
    ...
    cases: List[CaseReference] = Field(
        default_factory=list, description="Legal cases mentioned"
    )
    entities: List[LegalEntity] = Field(
        default_factory=list, description="Legal entities involved"
    )
    dates: List[LegalDate] = Field(
        default_factory=list, description="Important dates mentioned"
    )
    monetary_amounts: List[MonetaryAmount] = Field(
        default_factory=list, description="Financial amounts mentioned"
    )

    # Content analysis
    legal_topics: List[LegalTopic] = Field(
        default_factory=list, description="Legal practice areas covered"
    )
    key_quotes: List[str] = Field(
        default_factory=list, description="Significant quotes from the article"
    )

    # Legal classification
    jurisdiction: Optional[Jurisdiction] = Field(
        None, description="Primary jurisdiction"
    )
    court_level: Optional[CourtLevel] = Field(
        None, description="Court level if applicable"
    )
    practice_areas: List[str] = Field(
        default_factory=list, description="Specific practice areas"
    )

    # Structured metadata
    structured_data: Dict[str, Any] = Field(
        default_factory=dict, description="Additional structured information"
    )

    # Quality metrics
    extraction_confidence: float = Field(
        0.0, ge=0.0, le=1.0, description="Overall confidence in extraction"
    )
    completeness_score: float = Field(
        0.0, ge=0.0, le=1.0, description="How complete the extraction is"
    )
    ...
    def get_primary_entities(self, min_relevance: float = 0.7) -> List[LegalEntity]:
        ...
        return [
            ...
            if entity.relevance_score >= min_relevance
        ]
    ...
    def get_total_monetary_value(self, currency: str = "USD") -> float:
        ...
        return sum(
            amount.amount
            for amount in self.monetary_amounts
    ...
    def has_litigation_indicators(self) -> bool:
        ...
        litigation_topics = [
            LegalTopic.LITIGATION,
            LegalTopic.CRIMINAL_LAW,
            LegalTopic.TORT_LAW,
        ]
        litigation_entities = [
        ...
        has_litigation_topics = any(
            topic in self.legal_topics for topic in litigation_topics
        )
...
class ExtractionQuality(BaseModel):
    ...
    entity_accuracy: float = Field(
        ..., ge=0.0, le=10.0, description="Accuracy of entity extraction"
    )
    date_accuracy: float = Field(
        ..., ge=0.0, le=10.0, description="Accuracy of date extraction"
    )
    topic_relevance: float = Field(
        ..., ge=0.0, le=10.0, description="Relevance of identified topics"
    )
    completeness: float = Field(
        ..., ge=0.0, le=10.0, description="Completeness of extraction"
    )

    # Issues and recommendations
    issues_found: List[str] = Field(
        default_factory=list, description="Issues identified in extraction"
    )
    ...
    # Quality flags
    approved: bool = Field(
        False, description="Whether extraction meets quality standards"
    )
    needs_review: bool = Field(
        False, description="Whether extraction needs manual review"
    )
