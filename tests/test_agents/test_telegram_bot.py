...
class TestTelegramFormatter:
    ...
    @pytest.mark.asyncio
    async def test_format_article_for_telegram(
        self, formatter, sample_generated_article
    ):
    ...
    @pytest.mark.asyncio
    async def test_format_article_with_validation(
        self, formatter, sample_generated_article
    ):
        ...
        with patch.object(formatter, "_get_llm_telegram_format") as mock_llm:
            ...
            assert (
                "Quality" in telegram_message.content
                or "✅" in telegram_message.content
            )
...
class TestTelegramBotAgent:
    ...
    @pytest.mark.asyncio
    async def test_distribute_article_success(
        self, bot_agent, sample_generated_article
    ):
        ...
        with patch.object(bot_agent, "_send_with_retry") as mock_send:
            ...
            result = await bot_agent.distribute_article(
                sample_generated_article, validation
            )
    ...
    @pytest.mark.asyncio
    async def test_distribute_article_failure(
        self, bot_agent, sample_generated_article
    ):
    ...
    def test_should_retry_logic(self, bot_agent):
        ...
        client_error_result = DeliveryResult(
            success=False, message_id="test", error_code="400"
        )
    ...
    async def test_test_bot_setup(self, bot_agent):
        ...
        with patch("agents.telegram_bot.TelegramAPI") as mock_api_class:
            ...
            mock_api.send_message.return_value = DeliveryResult(
                success=True, message_id="test-msg", telegram_message_id=123
            )
    ...
    async def test_get_delivery_statistics(self, bot_agent):
        ...
        bot_agent.delivery_history = [
            ...
            DeliveryResult(
                success=False, message_id="msg3", error_code="400", response_time=0.5
            ),
        ]
    ...
    def test_clear_delivery_history(self, bot_agent):
        ...
        # Add some history
        bot_agent.delivery_history = [DeliveryResult(success=True, message_id="msg1")]
