...
class TestContentValidator:
    ...
    def high_quality_article(self) -> GeneratedArticle:
        ...
        return GeneratedArticle(
            ...
            tags=["contract law", "federal court", "commercial litigation", "legal precedent"],
...
class TestPlagiarismDetector:
    ...
    def test_check_plagiarism_patterns(self, detector):
        ...
        assert len(patterns) > 0
        assert any("academic content pattern" in pattern.lower() for pattern in patterns)
...
class TestQualityAssuranceAgent:
    ...
    async def test_validate_article_success(self, qa_agent, sample_generated_article):
        ...
        with patch.object(qa_agent.plagiarism_detector, 'check_plagiarism') as mock_plagiarism:
    ...
    @pytest.mark.asyncio
    async def test_validate_article_plagiarism_detected(self, qa_agent, sample_generated_article):
        ...
        with patch.object(qa_agent.plagiarism_detector, 'check_plagiarism') as mock_plagiarism:
            ...
            assert any("plagiarism" in issue.lower() for issue in result.critical_issues)
    
    @pytest.mark.asyncio
    async def test_validate_article_error_handling(self, qa_agent, sample_generated_article):
        ...
        with patch.object(qa_agent.plagiarism_detector, 'check_plagiarism') as mock_plagiarism:
            ...
            assert any("validation error" in issue.lower() for issue in result.critical_issues)
    
    @pytest.mark.asyncio
    async def test_batch_validate_articles(self, qa_agent, sample_generated_article):
        """Test batch validation of multiple articles."""
        articles = [sample_generated_article, sample_generated_article]  # Use same article twice
